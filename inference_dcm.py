...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    pnorm = pred_vol
    pnorm = pnorm * 0xff
    onorm = (orig_vol / np.max(orig_vol)) * 0xff
    overnorm = ((pnorm + onorm) / np.max(pnorm + onorm)) * 0xff
    d = datetime.date.today().strftime("%Y%m%d")
    t = datetime.datetime.now().strftime("%H%M%S")
    for n in range(len(slice_nums)):
        ndim = np.flip(overnorm[:,:,slice_nums[n]]).T.astype(np.uint8)
        pilim = Image.fromarray(ndim, mode="L").convert("RGBA").resize((300, 400))
        pimg.paste(pilim, box=(10+325*n, 400))
        pilim.save(os.path.join("..", "out", header.PatientID + "_" + d + "_" + t + '_slice' + str(slice_nums[n]) + '.png'),             format='png')
...
def get_series_for_inference(path):
    ...
    for i in dicoms:
        if (i[(0x0008,0x103e)].repval) == "'HippoCrop'":
...
if __name__ == "__main__":
    ...
    print(f"Found series of {volume.shape[2]} axial slices")
    print("HippoVolume.AI: Running inference...")
    inference_agent = UNetInferenceAgent(device="cpu", parameter_file_path=r"inference/model.pth")
    pred_label = inference_agent.single_volume_inference_unpadded(np.array(volume))
    pred_volumes = get_predicted_volumes(pred_label)
    dt = datetime.date.today().strftime("%Y%m%d")
    tm = datetime.datetime.now().strftime("%H%M%S")
